		...
		?>
            ...
            $(document).ready(function() {
                ...
                $('.activeMultiPop').CreateBubblePopup({
                    ...
                    align: 'center',
                    innerHtmlStyle: {color:'#FFFFFF', 'text-align':'center'},
                    innerHtml: 'Set <img src="images/ico_checked.png" align="absmiddle" /> to activate.', 
                    themeName: 'all-black',
                    themePath: '<?=__IMAGE__?>jquerybubblepopup-theme/'
                });
                ...
                $('.deleteMultiPop').CreateBubblePopup({
                    ...
                    align: 'center',
                    innerHtmlStyle: {color:'#FFFFFF', 'text-align':'center'},
                    innerHtml: 'Delete <img src="images/ico_checked.png" align="absmiddle" />', 
                    themeName: 'all-black',
                    themePath: '<?=__IMAGE__?>jquerybubblepopup-theme/'
                });
				...
				$('.add').click(function() {
					ajaxLoad("<?=__ROOT__?>/index.php?file=<?=$file?>&ajax=<?=$ajax?>&control=add-<?=$control?>","GET");
				});
				...
				$('.download').click(function() {
					...
					window.location = "<?=__ROOT__?>/index.php?file=download&download=<?=$control?>"+getURL;
				});
				...
				$('.print').click(function(){
					...
					window.open("<?=__ROOT__?>/index.php?file=print&print=stock-out" + getURL, "_blank", "toolbar=no, scrollbars=yes, resizable=no, top=" + top + ", left=" + left + ", width=" + w + ", height=" + (h - 110));
				});
                ...
                <tr class="line_20">
                    <td align="right" class="px_10" valign="top">
                    	<div class="float_left px_10" style="width:50%;" align="left">
							...
							<div style="margin-left: 28px;" class="green"><em>Search name, brand and plate</em></div>
                        </div>
                        
                        <div class="float_right px_10" style="width:50%;">
	                        Sort:&nbsp;
	                        <select id="filter_sort" name="filter_sort" class="selectoption thin_select pt_8">
	                            <? for ($i=0;$i<$stockoutsortdatacount;$i++): ?>
	                            <option value="<?=$stockoutsortdata[$i]['value']?>" <?=$filter_sort==$stockoutsortdata[$i]['value']?"selected=selected":""?>><?=$stockoutsortdata[$i]['name']?></option>
	                            <? endfor; ?>
	                        </select>
            ...
            <form action="<?=__ROOT__?>/index.php?file=process&process=<?=$control?>" method="post" enctype="multipart/form-data">
            <input type="hidden" value="&panel=inventory&section=<?=$control?>" name="return_link" />
            <input type="hidden" value="<?=$filter_link?>" name="filter_link" />
            <input type="hidden" value="requisition" name="table" />
            ...
            <table width="100%" border="0" cellpadding="5" cellspacing="0" class="list">
            	<tr style="background-color:#D7D7D7;" class="line_20">
                    ...
                    <th class="table_solid_top table_solid_bottom px_11 darkgray unselectable" align="center">Plate</th>
                    ...
                    <th class="table_solid_top table_solid_bottom px_11 darkgray unselectable" align="center">Quantity</th>
	        		<th class="table_solid_top table_solid_bottom px_11 darkgray unselectable" align="center">Stocks</th>
            ...
            if ($query_list_count > 0):
	            ...
	            for($x=0;$x<$query_list_count;$x++):
					...
					$info .= "<b class='orange'>Requisition Date:</b> $date <br />";
					
	                $info .= "<div class='spacer_5 clean'><!-- SPACER --></div>";
					$info .= "<b class='orange'>Price per unit:</b> &#8369; " . number_format($unit_price, 2, '.', ',') ." per $unit<br />";
					$info .= "<b class='orange'>Quantity:</b> $qty $unit<br />";	
	                $info .= "<b class='orange'>Total:</b> &#8369; " . number_format($total, 2, '.', ',') . " <br />";	
					
					$info .= "<div class='spacer_5 clean'><!-- SPACER --></div>";
					$info .= "<b class='orange'>Available Stock:</b> $stocks <br />";	
					
					$info .= "<div class='spacer_5 clean'><!-- SPACER --></div>";
					$info .=  $released_by != ' ' ? "<b class='orange'>Released By:</b> $released_by <br />":'';				
					$info .= "</div>";
	                ...
	                <script type="text/javascript">
	                    $(document).ready(function() {
	                        ...
	                        $('#edit_<?=$id?>').click(function(){
								ajaxLoad("<?=__ROOT__?>/index.php?file=<?=$file?>&ajax=<?=$ajax?>&control=edit-<?=$control?>&id=<?=$id?>","GET");
	                        });
	                    });
	                </script>
	                <tr class="line_20">
                        ...
                        <td class="table_solid_bottom px_12 unselectable" align="center"><span class="break"><?=stringLimit($date)?></span></td>
						<td class="table_solid_bottom px_12 unselectable" align="center"><span class="break"><?=stringLimit($plate)?></span></td>
                        ...
                        <td class="table_solid_bottom px_12 unselectable" align="center"><span class="break">&#8369; <?=number_format($unit_price, 2, '.', ',')?> per <?=$unit?></span></td>
						<td class="table_solid_bottom px_12 unselectable" align="center"><span class="break"><?=$qty?><? /* <?=$unit?> <?=$qty > 1 ? "s":"" */ ?></span></td>
	            ...
	            ?>   
	            	<tr class="line_20">
		            	<td class="table_solid_left table_solid_bottom table_solid_right px_11 unselectable darkgray bottom-left-radius" align="right" colspan="10"><b>Total Requisition:</b></td>
		            	<td style="background-color:#D7D7D7;" class="table_solid_bottom px_11 unselectable darkgray" align="center"><b>&#8369; <?=number_format($total_requisition, 2, '.', ',')?></b></td>
