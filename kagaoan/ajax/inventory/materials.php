...
$(document).ready(function() {
	...
	$('.add').click(function() {
		ajaxLoad("<?=__ROOT__?>/index.php?file=<?=$file?>&ajax=<?=$ajax?>&control=add-materials","GET");
	});
	...
	$('.download').click(function() {
		...
		window.location = "<?=__ROOT__?>/index.php?file=download&download=materials"+getURL;
	});
	...
	$('.print').click(function(){
		...
		window.open("<?=__ROOT__?>/index.php?file=print&print=materials" + getURL, "_blank", "toolbar=no, scrollbars=yes, resizable=no, top=" + top + ", left=" + left + ", width=" + w + ", height=" + (h - 110));
	});
});
    ...
    <tr class="line_20">
        <td align="right" class="px_10" valign="top">
        	<div class="float_left px_10" style="width:50%;" align="left">
				...
				<div style="margin-left: 28px;" class="green"><em>Search material no. and description</em></div>
			</div>

			<div class="float_right px_10" style="width:50%;">
			Sort:&nbsp;
			<select id="filter_sort" name="filter_sort" class="selectoption thin_select pt_8">
			    <? for ($i=0;$i<$materialsortdatacount;$i++): ?>
			    <option value="<?=$materialsortdata[$i]['value']?>" <?=$filter_sort==$materialsortdata[$i]['value']?"selected=selected":""?>><?=$materialsortdata[$i]['name']?></option>
			    <? endfor; ?>
			</select>
...
<form action="<?=__ROOT__?>/index.php?file=process&process=activate" method="post" enctype="multipart/form-data">
	<input type="hidden" value="&panel=inventory&section=materials" name="return_link" />
	<input type="hidden" value="<?=$filter_link?>" name="filter_link" />
	<input type="hidden" value="materials" name="table" />
	...
	<div class="table_title" align="center">
		<div align="left" class="px_16 float_left table_title_header">Inventory Materials</div>
	</div>
	...
	<table width="100%" border="0" cellpadding="5" cellspacing="0" class="list">
		<tr style="background-color:#D7D7D7;" class="line_20">
	        ...
	        <th class="table_solid_top table_solid_bottom px_11 darkgray unselectable" align="center">Gross Weight</th>
	        <th class="table_solid_top table_solid_bottom px_11 darkgray unselectable" align="center">Volume</th>
...
if ($query_list_count > 0):
	...
	for($x=0;$x<$query_list_count;$x++):
		...
		$info .= "<b class='orange'>Material:</b> $material <br />";
		$info .= $description != '' ? "<b class='orange'>Description:</b> $description <br />":"";
		$info .= "<b class='orange'>Gross Weight:</b> " . number_format($gross_weight, 2, '.', ',') . " <br />";
		$info .= "<b class='orange'>Volume:</b> " . number_format($volume,3,'.',',') . "<br />";					
		$info .= "</div>";
		...
		$(document).ready(function() {
			...
			$('#edit_<?=$id?>').click(function(){
				ajaxLoad("<?=__ROOT__?>/index.php?file=<?=$file?>&ajax=<?=$ajax?>&control=edit-materials&id=<?=$id?>","GET");
				});
		});
		</script>
    	...
    	<tr class="line_20">
			...
			<td class="table_solid_bottom px_12 unselectable" align="center"><span class="break"><?=number_format($gross_weight, 2, '.', ',')?></span></td>
			<td class="table_solid_bottom px_12 unselectable" align="center"><span class="break"><?=number_format($volume,3,'.',',')?></span></td>		
