			<?php
			$data = $connect->single_result_array("SELECT * FROM truck_driver WHERE id = '{$id}'");
			
			$assigned = $data['assigned'] == 'D' || $data['assigned'] == '' ? date("Y-m-d H:i:s"):$data['assigned'];
			$select = new Select();
			...
			$select_plate =  $select->option_query(
				...
				'id',							// value='$value'
				'plate',						// option name
				$truck_id,						// default selected value
			...
			$select_driver =  $select->option_query(
				...
				$driver_id,						// default selected value
		...
		?>
        	...
        	<form method="post" enctype="multipart/form-data" action="<?=__ROOT__?>/index.php?file=action&action=edit-driver" id="editForm">
        	...
        	<div style="width:100%;" align="left">
                ...
                <table width="100%" border="0" cellpadding="0" cellspacing="5" class="table_solid_bottom table_solid_left table_solid_right radius_bottom_10">
                    ...
                    <tr>
                        ...
                        <td><input id="assigned" name="assigned" type="text" class="inputtext default_inputtext datepicker" maxlength="50" value="<?=$assigned?>" /></td>
