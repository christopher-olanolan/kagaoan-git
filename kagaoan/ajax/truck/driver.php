		...
		?>
            ...
            $(document).ready(function() {
                ...
                $('.editPop').CreateBubblePopup({
                    ...
                    innerHtml: 'Click to edit assigned driver.', 
                    themeName: 'all-black',
                ...
                $('.addPop').CreateBubblePopup({
                    ...
                    align: 'center',
                    innerHtmlStyle: {color:'#FFFFFF', 'text-align':'center'},
                    innerHtml: 'Click to assign driver.', 
                    themeName: 'all-black',
                    themePath: '<?=__IMAGE__?>jquerybubblepopup-theme/'
                });
                ...
                $('.deletePop').CreateBubblePopup({
                    ...
                    align: 'center',
                    innerHtmlStyle: {color:'#FFFFFF', 'text-align':'center'},
                    innerHtml: 'Click to delete assigned driver.', 
                    themeName: 'all-black',
                    themePath: '<?=__IMAGE__?>jquerybubblepopup-theme/'
                });
                ...
                $('.activatePop').CreateBubblePopup({
                    ...
                    align: 'center',
                    innerHtmlStyle: {color:'#FFFFFF', 'text-align':'center'},
                    innerHtml: 'Click to activate assigned driver.', 
                    themeName: 'all-black',
                    themePath: '<?=__IMAGE__?>jquerybubblepopup-theme/'
                });
                ...
                $('.activeMultiPop').CreateBubblePopup({
                    ...
                    align: 'center',
                    innerHtmlStyle: {color:'#FFFFFF', 'text-align':'center'},
                    innerHtml: 'Set <img src="images/ico_checked.png" align="absmiddle" /> assigned driver(s) to active.', 
                    themeName: 'all-black',
                    themePath: '<?=__IMAGE__?>jquerybubblepopup-theme/'
                });
                ...
                $('.deleteMultiPop').CreateBubblePopup({
                    ...
                    align: 'center',
                    innerHtmlStyle: {color:'#FFFFFF', 'text-align':'center'},
                    innerHtml: 'Delete <img src="images/ico_checked.png" align="absmiddle" /> assigned driver(s).', 
                    themeName: 'all-black',
                    themePath: '<?=__IMAGE__?>jquerybubblepopup-theme/'
                });
				...
				$('.add').click(function() {
					ajaxLoad("<?=__ROOT__?>/index.php?file=<?=$file?>&ajax=<?=$ajax?>&control=add-driver","GET");
				});
				...
				$('.download').click(function() {								
					...
					window.location = "<?=__ROOT__?>/index.php?file=download&download=driver"+getURL;
				});
				...
				$('.print').click(function(){
					...
					window.open("<?=__ROOT__?>/index.php?file=print&print=driver" + getURL, "_blank", "toolbar=no, scrollbars=yes, resizable=no, top=" + top + ", left=" + left + ", width=" + w + ", height=" + (h - 110));
				});
            });
                ...
                <tr class="line_20">
                    <td align="right" class="px_10" valign="top">
                    	<div class="float_left px_10" style="width:50%;" align="left">
							...
							<div style="margin-left: 28px;" class="green"><em>Search driver, type, model or plate no.</em></div>
                        </div>
                        
                        <div class="float_right px_10" style="width:50%;">
	                        Sort:&nbsp;
	                        <select id="filter_sort" name="filter_sort" class="selectoption thin_select pt_8">
	                            <? for ($i=0;$i<$driversortdatacount;$i++): ?>
	                            <option value="<?=$driversortdata[$i]['value']?>" <?=$filter_sort==$driversortdata[$i]['value']?"selected=selected":""?>><?=$driversortdata[$i]['name']?></option>
	                            <? endfor; ?>
	                        </select>
            ...
            <form action="<?=__ROOT__?>/index.php?file=process&process=manage-driver" method="post" enctype="multipart/form-data">
            ...
            <table width="100%" border="0" cellpadding="6" cellspacing="0">
                <tr class="line_20">
                    <td align="left" valign="top">
						...
						<input type="button" class="button small_button add" name="add" id="add" value="Assign New Driver" />
            ...
            <table width="100%" border="0" cellpadding="5" cellspacing="0" class="list">
            	<tr style="background-color:#D7D7D7;" class="line_20">
	        		...
	        		<th class="table_solid_top table_solid_bottom px_11 darkgray unselectable" align="center">Assigned Driver</th>
	        		<th class="table_solid_top table_solid_bottom px_11 darkgray unselectable" align="center">Date Assigned</th> 
            ...
            if ($query_list_count > 0):
	            for($x=0;$x<$query_list_count;$x++):
	                ...
	                $info .= "<b class='orange'>Type:</b> $truck_type <br />";
	                
					$info .= "<div class='spacer_5 clean'><!-- SPACER --></div>";
					$info .= "<b class='orange'>Assigned Driver:</b> $driver <br />";
					$info .= "<b class='orange'>Date Assigned:</b> $created <br />";
					$info .= "</div>";
	                ...
	                <script type="text/javascript">
	                    $(document).ready(function() {
	                        ...
	                        $('#edit_<?=$id?>').click(function(){
								ajaxLoad("<?=__ROOT__?>/index.php?file=<?=$file?>&ajax=<?=$ajax?>&control=edit-driver&truck_id=<?=$truck_id?>&driver_id=<?=$driver_id?>&id=<?=$id?>","GET");
	                        });

	                        $('#add_<?=$x.'_'.$truck_id?>').click(function(){
								ajaxLoad("<?=__ROOT__?>/index.php?file=<?=$file?>&ajax=<?=$ajax?>&control=edit-driver&truck_id=<?=$truck_id?>","GET");
	                        });
	                    });
	                </script>
	                <tr class="line_20">
	                    <td width="1%" class="table_solid_left table_solid_right table_solid_bottom <?=$x==$query_list_count-1?'bottom-left-radius':''?>" align="center">
	                        <?=$id!=""?'<input type="checkbox" class="checkbox" name="action[checkbox][]" value="'.$id.'" />':''?>
	                    </td>
						...
						<td width="75" class="table_solid_right table_solid_bottom px_11 unselectable <?=$x==$query_list_count-1?'bottom-right-radius':''?>" align="center">
							...
							if ($id != ''):
	                        	...
	                        	?><input type="button" class="clean float_right addPop ico ico_add" name="action[single-add]" value="<?=$truck_id?>" id="add_<?=$x.'_'.$truck_id?>" /><?
	                        endif;
	                        ?>
