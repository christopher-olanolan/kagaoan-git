...
$filter_link .= $filter_delivered != '' ? "&filter_delivered=".$filter_delivered:"";
...
function loadURL(page,bottom){
	...
	var input_filter_delivered = $('#filter_delivered option:selected').val();
	...
	getURL +="&filter_delivered="+input_filter_delivered;
...
$(document).ready(function() {
	...
	$('.add').click(function() {
		ajaxLoad("<?=__ROOT__?>/index.php?file=<?=$file?>&ajax=<?=$ajax?>&control=add-transaction","GET");
	});
	...
	$('.download').click(function() {
		...
		window.location = "<?=__ROOT__?>/index.php?file=download&download=transaction"+getURL;
	});
	...
	$('.print').click(function(){
		...
		window.open("<?=__ROOT__?>/index.php?file=print&print=transaction" + getURL, "_blank", "toolbar=no, scrollbars=yes, resizable=no, top=" + top + ", left=" + left + ", width=" + w + ", height=" + (h - 110));
	});
        ...
        <td align="right" class="px_10" valign="top">
			...
			<div class="float_right px_10" style="width:50%;">
				...
				<select id="filter_sort" name="filter_sort" class="selectoption thin_select pt_8">
				    <? for ($i=0;$i<$transactionsortdatacount;$i++): ?>
				    <option value="<?=$transactionsortdata[$i]['value']?>" <?=$filter_sort==$transactionsortdata[$i]['value']?"selected=selected":""?>><?=$transactionsortdata[$i]['name']?></option>
				    <? endfor; ?>
				</select>
			...
			<div id="advanced" class="float_left" style="margin-left: 22px;" align="left">
        		...
        		<div class="float_left px_10 marg_right_5">
        			Delivery Status:<br />
	        		<select id="filter_delivered" name="filter_delivered" class="selectoption thin_select pt_8">
	        			<option value="all" <?=$filter_delivered=="all"?"selected=selected":""?>>All</option>
					    <option value="0" <?=$filter_delivered=="0"?"selected=selected":""?>>In-transit</option>
					    <option value="1" <?=$filter_delivered=="1"?"selected=selected":""?>>Delivered</option>
					</select>
				</div>
...
<input type="hidden" value="&panel=transaction&section=manage" name="return_link" />
...
<table width="100%" border="0" cellpadding="6" cellspacing="0">
    <tr class="line_20">
        <td align="left" valign="top">
			...
			<input type="button" class="button small_button add" name="add" id="add" value="New Transaction" />
...
<div id="PrintArea" class="PrintArea">
	...
	<table width="100%" border="0" cellpadding="5" cellspacing="0" class="list">
		<tr style="background-color:#D7D7D7;" class="line_20">
			...
			<th class="table_solid_top table_solid_bottom px_11 darkgray unselectable" align="center">Status</th>
...
if ($query_list_count > 0):
	...
	for($x=0;$x<$query_list_count;$x++):
	    ...
	    $transaction_status = $query_list[$x]['active'];
		...
		$info .= "<div class='spacer_5 clean'><!-- SPACER --></div>";
		$info .= "<b class='orange'>No. of CS:</b> $cs <br />";
		$info .= "<b class='orange'>Rate:</b> &#8369; " . number_format($rate, 2, '.', ',') . "<br />";
		$info .= "<b class='orange'>Delivery Status:</b> $delivery_status <br />";
		...
		$(document).ready(function() {
			...
			$('#edit_<?=$id?>').click(function(){
				ajaxLoad("<?=__ROOT__?>/index.php?file=<?=$file?>&ajax=<?=$ajax?>&control=edit-transaction&id=<?=$id?>&filter_plate=<?=$filter_plate?>&filter_from=<?=$filter_from?>&filter_to=<?=$filter_to?>","GET");
			});
		});
	    </script>
	    <tr class="line_20">
			...
			<td class="table_solid_bottom px_12 unselectable" align="center"><span class="break"><?=$delivery_status?></span></td> 
	...
	<? endfor; ?>   
		<tr class="line_20">
			<td class="table_solid_left table_solid_bottom table_solid_right px_11 unselectable darkgray bottom-left-radius" align="right" colspan="8"><b>Total Transaction:</b></td>
			...
			<td style="background-color:#D7D7D7;" class="table_solid_bottom px_11 unselectable darkgray" align="center"><b>&#8369; <?=number_format($total_transaction, 2, '.', ',')?></b></td>
			<td class="table_solid_right table_solid_bottom px_11 unselectable  darkgray bottom-right-radius" align="left" colspan="2">&nbsp;</td>
		</tr>
	</table>
</div>  <!-- END PRINT -->
